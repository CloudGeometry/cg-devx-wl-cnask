...
>(
  findMany: (prisma_args: PrismaFindManyArguments<Cursor>) => Promise<Record[]>,
  aggregate: () => Promise<number>,
  args: ConnectionArguments = {},
  prisma_args: PrismaFindManyArguments<Cursor>,
  ...
  if (isForwardPagination(args)) {
    ...
    const skip = args.offset || (cursor ? 1 : undefined);

    // Execute the underlying query operations
    records = await findMany({ cursor, take, skip, ...prisma_args });
